...
FORWARD_VIDEOS = config['forward_videos']
...
def randomly_forward_to_group(reel, client):
    forwarded = False
    if random.choice([True, False]) and FORWARD_VIDEOS:
        group = random.choice(GROUPS)
        client.direct_send(text="Check out this reel!", user_ids=[client.user_id_from_username(group)])
        forwarded = True
    return forwarded
    ...
    while len(downloaded_reels) < num_reels:
        ...
        for reel in reels:
            ...
            if os.path.exists(media_path):
                ...
                forwarded = randomly_forward_to_group(reel, cl)
                if forwarded:
                    logging.info(f"Forwarded reel: {reel.pk} to group")
...
def upload_reels_with_new_descriptions(unuploaded_reels):
    ...
    for reel_file in unuploaded_reels:
        ...
        sleep_time = random_sleep(10, 60)  # Random sleep before next upload
        ...
        if (current_time - last_scrape_time) >= SCRAPE_INTERVAL_MINUTES * 60:
            if SCRAPING_ENABLED:
                for profile in config['scraping']['profiles'].split():
                    ...
                    for reel in reels:
                        random_sleep(10, 60)  # Random sleep before processing each reel
                        randomly_like_and_comment(reel, cl)
                        randomly_forward_to_group(reel, cl)
