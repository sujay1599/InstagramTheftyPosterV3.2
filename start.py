...
# Adding user-defined comments and groups for forwarding videos
leave_comment = input('Leave comment on scraped videos? (true/false): ').lower() == 'true'
config['leave_comment'] = leave_comment

if leave_comment:
    config['comments'] = input('Enter comments (comma separated): ').split(',')

forward_videos = input('Forward videos to groups? (true/false): ').lower() == 'true'
config['forward_videos'] = forward_videos

if forward_videos:
    config['groups'] = input('Enter group names (comma separated): ').split(',')
